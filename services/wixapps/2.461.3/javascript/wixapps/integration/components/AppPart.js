...
define.component("wixapps.integration.components.AppPart", function (componentDefinition) {
	...
	def.methods({
        ...
        initApp: function() {
            ...
            this._allowHeightResize = this._configByFormatAndView(null, 'allowHeightResize', false);
            ...
            var minWidth = this._configByFormatAndView(null, 'minWidth', 0);
        ...
        getHelpId: function(){
            var helpId = this._configByFormatAndView(null, 'helpId', '');
            if(!!helpId) {
                return helpId;
            }
        ...
        showView: function(dataItem, viewName, eventHandlers, vars) {
            ...
            this._allowHeightResize = this._configByFormatAndView(viewName, 'allowHeightResize', this._allowHeightResize);
            if(!this._allowHeightResize) {
                this._resizableSides = [Constants.BaseComponent.ResizeSides.LEFT, Constants.BaseComponent.ResizeSides.RIGHT];
                this._updateHeight();
            } else {
                this._resizableSides = [Constants.BaseComponent.ResizeSides.LEFT, Constants.BaseComponent.ResizeSides.RIGHT, Constants.BaseComponent.ResizeSides.TOP, Constants.BaseComponent.ResizeSides.BOTTOM];
		...
		/**
		 * Use with caution! Don't forget to reset!
		 * @param value
		 */
		setPreventLogicRun: function(value){
			this._preventLogicFromRunning = value;
		},

		_runLogic: function() {
			if(this._preventLogicFromRunning) { return; }
		...
		//use this to ask appParts that are children of other components (like in zoom) to resize themselves when needed
		invalidateAppPartSize: function(){
			this._lifecycleManager.performAction(function () {
				this.fireEvent('invalidateSize');
			}.bind(this));
		},
        ...
        _configOverride: function(configKey, keyName, paramName) {

            var resultValue;

            if(this._partDef[configKey] &&
                keyName in this._partDef[configKey] &&
                paramName in this._partDef[configKey][keyName]) {

                resultValue = this._partDef[configKey][keyName][paramName];
            }
            return resultValue;
        },

        _configByFormatAndView: function(viewName, paramName, defaultValue) {
            var resultValue = defaultValue;

            var configByFormat = this._configOverride("configByFormat", this._formatName, paramName);
            if(configByFormat!==undefined) {
                return configByFormat;
            } else if(viewName) {
                var configByView = this._configOverride("configByView", viewName, paramName);
                if(configByView!==undefined) {
                    return configByView;
                }
            }

            if(paramName in this._partDef) {
